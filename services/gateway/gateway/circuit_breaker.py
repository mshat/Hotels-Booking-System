...
class CircuitBreakerItem:
    ...
    def __str__(self):
        return f'CircuitBreakerItem: ' \
               f'{self._url} {self._fallback}, state: {self._state}, time: {self._opening_time},' \
               f' errors: {self._error_counter}'
...
class CircuitBreaker:
    def __init__(self, redis_instance, errors_num_before_opening=1, health_check_timeout=5, polling_sleep_time=1):
        ...
        self._logger = Logger(f'CircuitBreaker')
        self._logger.info(f'CircuitBreaker initialized')

    def _add_item_to_db(self, url: Url) -> None:
        l = self._logger.child('Method _add_item_to_db')
        l.info(f'adding item {url} to db')
        ...
        l.info(f'DEBUG Я МЕНЯЮ ЛАСТ ТАЙМ В АДД ИТЕМ ТО ДБ')
    ...
    def _update_item_in_db(self, url: Url, state: int = None, time_: float = None) -> None:
        l = self._logger.child('Method _update_item_in_db')
        l.info(f'update item {url} with: state {state} time {time_}')
        ...
        if time_:
            l.info(f'DEBUG Я МЕНЯЮ ЛАСТ ТАЙМ В _update_item_in_db')
    ...
    def _send_request(self, request_method, uri: Uri, headers=None, data=None, **kwargs) -> Response | Fallback:
        ...
        l.info(f'Send_request {request_method.__name__.upper()} to {uri} with {headers=} {data=}')
        l.info(f"Tracked items: {self._tracked_items.values()}")
        url = Url(uri)
        ...
        try:
            l.info(f'DEBUG NOW I SENDING REQUEST: {uri.str=} {headers=} {data=}')
